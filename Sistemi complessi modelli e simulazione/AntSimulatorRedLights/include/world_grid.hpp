...
struct WorldGrid : public Grid<WorldCell>
{
	...
	void addFood(sf::Vector2f pos, uint32_t quantity)
	{
		...
		cell.permanent[1] = true; // true = food does not expire
		// increase ammount of food in the scene
		food_count += quantity;
	}
	...
	// check if the ant is under light
	bool isOnLight(sf::Vector2f pos) const
	{
		return getCst(pos).light;
	}

	void pickFood(sf::Vector2f pos)
	{
		get(pos).pick();
		// decrese ammount of food in the scene
		food_count--;
	}
